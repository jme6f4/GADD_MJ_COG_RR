...
# # D) readModels/summaries ####
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
ReadModels22rr <- readModels(target = paste0(getwd(),"/Work/Documentation/githubRR/",Model),
                           filefilter = c("_1_19|_2_19"))
library(plyr)
Summary22rr <- as_tibble(do.call("rbind.fill", sapply(ReadModels22rr, "[", "summaries")))
detach("package:plyr", unload=TRUE)
FitProblems_22rr <- Summary22rr %>% dplyr::filter(is.na(AIC))
ifelse(dim(FitProblems_22rr)[1]!=0, "CHECK MODELS", "ok")
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
ci.unstandardized22rr <- sapply(sapply(ReadModels22rr, 
                                  "[", "parameters"), 
                           "[", "ci.unstandardized")
lapply(names(ci.unstandardized22rr), function(element) {
  ci.unstandardized22rr[[element]]$filename <<- element
  })
Estimates_22rr_UnstCI <- as_tibble(do.call("rbind", ci.unstandardized22rr)) %>%
  dplyr::select(-filename)

se.unstandardized22rr <- sapply(sapply(ReadModels22rr, 
                                  "[", "parameters"), 
                           "[", "unstandardized")
lapply(names(se.unstandardized22rr), function(element) {
  se.unstandardized22rr[[element]]$filename <<- element
  })
Estimates_22rr_UnstSE <- as_tibble(do.call("rbind", se.unstandardized22rr))
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
unstandardized22rr <- list(Estimates_22rr_UnstCI, Estimates_22rr_UnstSE) %>%
  purrr::reduce(full_join)

Estimates_22rr_Unst <- unstandardized22rr  %>%
...
Estimates_22rr_Tab_Unst  <- Estimates_22rr_Unst %>%
  dplyr::select(Column, Row, 
...
TableEstimates_22rr_Unst <- Estimates_22rr_Tab_Unst %>%
  dplyr::select(Column, Row, DisplayFull)  %>%
  filter(grepl(")",Column)) %>%
  spread(key = Column, value = DisplayFull) 

TableEstimates_22rr_Unadj <- Estimates_22rr_Tab_Unst %>%
  dplyr::select(Column, Row, DisplayUnadj)  %>%
  filter(grepl(")",Column)) %>%
  spread(key = Column, value = DisplayUnadj) 

View(Estimates_22rr_Unst)
View(TableEstimates_22rr_Unadj)
View(TableEstimates_22rr_Unst)
...
Model <- "22_MultiLev"
date <- "190618rr"
write_csv(x = TableEstimates_22rr_Unst,
          na = "",
